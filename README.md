...
后端基于 FastAPI、Flow YAML 状态机、LLM TurnPlanner 和 Action 执行器。模型负责理解和生成，流程模板负责必要信息收集。
...
- 知识问答：生成 evidence、missing fields、route team 和 reply draft。
...
1. `subscription_support`
   ...
   - 本地生成 `support_ticket_id`、`route_team`、`issue_summary`、`reply_draft`
